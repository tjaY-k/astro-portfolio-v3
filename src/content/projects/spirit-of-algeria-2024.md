...
tech: ["disguise", "Projection Mapping", "360° Dome Projection", "Media Servers", "Immersive Experiences", "National Commemoration"]
...
The map ran on the inside of the dome, with the audience beneath it: 64 projectors covering one continuous surface. A closed curve leaves no edge on which to hide a seam, so every blend has to meet itself — and on a concave surface, light thrown at one side lands on the other, lifting the black level across the whole image. Neither problem exists on a façade.
